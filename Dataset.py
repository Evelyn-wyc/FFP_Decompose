...
class Label_loader:
    def __init__(self, cfg, video_folders):
        # assert cfg.dataset in ('ped2', 'avenue', 'shanghaitech', 'wood_ano'), f'Did not find the related gt for \'{cfg.dataset}\'.'
        self.cfg = cfg
        self.name = cfg.dataset
        self.frame_path = cfg.test_data
        self.mat_path = f'{cfg.data_root + self.name}/{self.name}_label.mat'
    ...
    def load_ucsd_avenue(self):
        abnormal_events = scio.loadmat(self.mat_path)['gt']
        ...
        for i in range(abnormal_events.shape[0]):
            ...
            for j in range(one_abnormal.shape[0]):
                start = one_abnormal[j, 0] - 1
