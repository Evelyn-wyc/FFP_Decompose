...
from function import psnr_error
import Dataset
        ...
        for i, folder in enumerate(video_folders):
            ...
            if not model:
                ...
                if cfg.show_curve:
                    ...
                    curve_writer = cv2.VideoWriter(f'results/{name}_curve.avi', fourcc, 30, cfg.img_size)

                    js = []
                    plt.clf()
                    ax = plt.axes(xlim=(0, len(dataset)), ylim=(25, 40))
                    line, = ax.plot([], [], '-b')

                if cfg.show_heatmap:
                    heatmap_writer = cv2.VideoWriter(f'results/{name}_heatmap.avi', fourcc, 30, cfg.img_size)
                    generate_writer = cv2.VideoWriter(f'results/{name}_generate.avi', fourcc, 30, cfg.img_size)
            ...
            for j, clip in enumerate(dataset):
                ...
                if not model:
                    if cfg.show_curve:
                        ...
                        # cv2.imshow('psnr curve', curve_img)
                        # cv2.waitKey(1)

                        curve_writer.write(curve_img)
                        # 保存图片
                        cv2.imwrite('results/curve_img2.jpg', curve_img)
                    ...
                    if cfg.show_heatmap:
                        ...
                        gene_frame = ((G_frame + 1) * 127.5).squeeze().permute(1, 2, 0).cpu().detach().numpy().astype('uint8')

                        cv2.imshow('generate frames', gene_frame)
                        cv2.waitKey(1)

                        generate_writer.write(gene_frame)  # Write generated frames.
